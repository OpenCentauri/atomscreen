...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    ui.global::<Filesystem>().on_list_files(move || {
        ...
        slint::spawn_local(async move {
            if let Ok(mut files) = moonraker_connection.get_file_list().await
            {
                ...
                let converted_files : Vec<MoonrakerFileTest> = files.iter().map(|f|  {
                    MoonrakerFileTest {
                ...
                ui.global::<Filesystem>().set_files(ModelRc::new(Rc::new(VecModel::from(converted_files))));
    ...
    ui.global::<Filesystem>().on_download_thumbnail(move |global_index | {
        ...
        slint::spawn_local(async move {
            ...
            let mut unwrapped_files : Vec<MoonrakerFileTest> = non_optional_files.iter().collect();
            ...
            if global_index < unwrapped_files.len()
            {
                ...
                // TODO: Error handling
                let files = moonraker_connection.get_thumbnails_for_file(&file.path).await.unwrap();
                ...
                {
                    let data = moonraker_connection.download_thumbnail(&target.thumbnail_path).await.unwrap();

                    // TODO: Error handling
                    let image = image::load_from_memory(&data).unwrap().into_rgba8();
                    let shared_buf: SharedPixelBuffer<Rgba8Pixel> = SharedPixelBuffer::clone_from_slice(image.as_raw(), image.width(), image.height());
