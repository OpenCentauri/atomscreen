    ...
    pub fn complete_event(&mut self, event: OptionalPrinterEvent) -> PrinterEvent
    {
        match event {
            ...
            OptionalPrinterEvent::TemperatureSensor(named_sensor) => {
                let index = self.temperature_sensors.iter().position(|sensor| sensor.name == named_sensor.name);

                let sensor = match index {
                    Some(index) => {
                        self.temperature_sensors[index].sensor.overlay(named_sensor.sensor);
                        self.temperature_sensors[index].clone()
                    },
                    None => {
                        let mut new_sensor = TemperatureSensor::default();
                        new_sensor.overlay(named_sensor.sensor);
                        let new_named_sensor = NamedTemperatureSensor { name: named_sensor.name, sensor: new_sensor };
            ...
            OptionalPrinterEvent::TemperatureFan(named_fan) => {
                let index = self.temperature_fans.iter().position(|fan| fan.name == named_fan.name);
                ...
                let fan = match index {
                    ...
                    None => {
                        ...
                        let new_named_fan = NamedTemperatureFan { name: named_fan.name, fan: new_fan };
            ...
            OptionalPrinterEvent::FilamentSwitchSensor(named_sensor) => {
                let index = self.filament_switch_sensors.iter().position(|sensor| sensor.name == named_sensor.name);

                let sensor = match index {
                    Some(index) => {
                        self.filament_switch_sensors[index].sensor.overlay(named_sensor.sensor);
                        self.filament_switch_sensors[index].clone()
                    },
                    None => {
                        let mut new_sensor = FilamentSwitchSensor::default();
                        new_sensor.overlay(named_sensor.sensor);
                        let new_named_sensor = NamedFilamentSwitchSensor { name: named_sensor.name, sensor: new_sensor };
            ...
            OptionalPrinterEvent::OutputPin(named_pin) => {
                let index = self.output_pins.iter().position(|pin| pin.name == named_pin.name);
                ...
                let pin = match index {
                    ...
                    None => {
                        ...
                        let new_named_pin = NamedOutputPin { name: named_pin.name, pin: new_pin };
