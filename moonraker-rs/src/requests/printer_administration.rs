...
use crate::{connector::read_deserialize::MoonrakerEventNotifyStatusUpdate, error::Error, moonraker_connection::MoonrakerConnection};

pub trait PrinterAdministrationRequestHandler
{
    async fn list_printer_objects(&self) -> Result<PrinterObjectListResponse, Error>;
    async fn subscribe_to_printer_objects(&self, objects: Vec<String>) -> Result<PrinterObjectsSubscribeResult, Error>;
}
...
impl PrinterAdministrationRequestHandler for MoonrakerConnection
{
    ...
    async fn subscribe_to_printer_objects(&self, objects: Vec<String>) -> Result<PrinterObjectsSubscribeResult, Error>
    {
        ...
        self.send_request("printer.objects.subscribe", Some(args)).await
    }
}
