...
use tokio::{io::ReadHalf, sync::{broadcast::Sender, Mutex}};

use crate::{cache::Cache, connector::{read_deserialize::{JsonRpcResponse, MoonrakerEventParameters, MoonrakerNotifyProcStatUpdate}, websocket_write::OutboundMessage}, error::Error, moonraker_connection::{MoonrakerConnection, MoonrakerReply, WebsocketEvent}, printer_objects::*};
...
pub(crate) async fn moonraker_reader_connection_loop(
    ...
    let mut data = MoonrakerConnectionReadLoop::new(inbound_sender, outbound_sender, websocket_reader, cache);
...
impl MoonrakerConnectionReadLoop {
    ...
    pub async fn connection_loop(&mut self) {
        loop 
        {
            ...
            let mut frame = match self.websocket_reader.read_frame(&mut move |x| {
                outbound_sender.send(Arc::new(OutboundMessage::RawFrame(Mutex::new(Some(x))))).expect("Failed to internally send a raw frame event");
                ...
                Err(e) => {
                    ...
                    self.inbound_sender.send(Arc::new(WebsocketEvent::Error(e.to_string()))).expect("Failed to internally send an error event");
            ...
            if let Err(e) = match frame.opcode {
                ...
                _ => Err(Error::Unknown(format!("Received unsupported websocket frame {:?}", frame.opcode))),
            }
            {
                // TODO: Erorr handling
        ...
        match data {
            JsonRpcResponse::MethodResponse(method_response) => {
                ...
                else 
                {
                    ...
                    let _ = self.inbound_sender.send(Arc::new(WebsocketEvent::MoonrakerReply(reply)));
                }
            } 
                ...
                match notification.params
                {
                    MoonrakerEventParameters::NotifyStatusUpdate(status_update) => {
                        for event in status_update.events
                        {
                            ...
                            self.inbound_sender.send(Arc::new(WebsocketEvent::MoonrakerEvent(MoonrakerEvent::NotifyStatusUpdate(unlocked_cache.complete_event(event))))).expect("Failed to internally send a moonraker status update event");
                        }
                    },
