...
impl<'de> Deserialize<'de> for JsonRpcResponse {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if value.get("method").is_some() {
            let notification =
                JsonRpcNotification::deserialize(value).map_err(serde::de::Error::custom)?;
            Ok(JsonRpcResponse::Notification(notification))
        } else {
            let method_response =
                JsonRpcMethodResponse::deserialize(value).map_err(serde::de::Error::custom)?;
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        for (object_name, object_value) in helper {
            ...
            let event = match first_part_of_name {
                "webhooks" => OptionalPrinterEvent::Webhooks(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "motion_report" => OptionalPrinterEvent::MotionReport(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "gcode_move" => OptionalPrinterEvent::GcodeMove(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "toolhead" => OptionalPrinterEvent::Toolhead(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "extruder" => OptionalPrinterEvent::Extruder(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "heater_bed" => OptionalPrinterEvent::HeaterBed(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "fan" => OptionalPrinterEvent::Fan(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "idle_timeout" => OptionalPrinterEvent::IdleTimeout(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "virtual_sdcard" => OptionalPrinterEvent::VirtualSdcard(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "print_stats" => OptionalPrinterEvent::PrintStats(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "display_status" => OptionalPrinterEvent::DisplayStatus(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                "temperature_sensor" => {
                    OptionalPrinterEvent::TemperatureSensor(NamedOptionalTemperatureSensor {
                        name: last_part_of_name.to_string(),
                        sensor: serde_json::from_value(object_value)
                            .map_err(serde::de::Error::custom)?,
                    })
                }
                "temperature_fan" => {
                    OptionalPrinterEvent::TemperatureFan(NamedOptionalTemperatureFan {
                        name: last_part_of_name.to_string(),
                        fan: serde_json::from_value(object_value)
                            .map_err(serde::de::Error::custom)?,
                    })
                }
                "filament_switch_sensor" => {
                    OptionalPrinterEvent::FilamentSwitchSensor(NamedOptionalFilamentSwitchSensor {
                        name: last_part_of_name.to_string(),
                        sensor: serde_json::from_value(object_value)
                            .map_err(serde::de::Error::custom)?,
                    })
                ...
                "exclude_object" => OptionalPrinterEvent::ExcludeObject(
                    serde_json::from_value(object_value).map_err(serde::de::Error::custom)?,
                ),
                _ => {
